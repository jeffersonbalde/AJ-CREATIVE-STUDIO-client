...
const GoogleLoginButton = ({ onSuccess, onError, disabled = false, loading: externalLoading = false }) => {
  const [internalLoading, setInternalLoading] = useState(false);
  const [isHovered, setIsHovered] = useState(false);
  const loading = externalLoading || internalLoading;
  
  useEffect(() => {
    console.log('🔵 GoogleLoginButton MOUNTED/RENDERED', { disabled, loading, externalLoading, internalLoading });
  }, [disabled, loading, externalLoading, internalLoading]);

  const handleClick = async (e) => {
    e.preventDefault();
    e.stopPropagation();
    console.log('🔵 GoogleLoginButton clicked!', { disabled, loading });
    ...
    showAlert.processing('', '', {
      ...
      html: `
        <div style="display:flex;flex-direction:column;align-items:center;gap:6px;">
          <div style="font-size:14px;font-weight:800;color:#111;line-height:1.2;">Signing in with Google...</div>
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        const error = new Error(data?.message || 'Google login failed. Please try again.');
      ...
      if (err.code === 'auth/popup-closed-by-user') {
        ...
        setTimeout(() => {
          showAlert.info(
            'Sign-in Cancelled',
            'You cancelled the Google sign-in. Please try again if you want to continue.',
            {
              width: 360,
              padding: '0.9rem 1rem',
